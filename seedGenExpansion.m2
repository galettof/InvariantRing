...
seedMinimal := (Seeds, candidate, startingIndex) -> (
	for i to #Seeds - 1 do (	-- Iterate through all seeds
		candMinimal := true;	-- We start by assuming our candidate & seed are divisible by each other
		seedMinimal := true;
		for k to #seedMinimal - 1 do (
			if (candidate#k > Seeds#i) then (
				candMinimal = false; -- If our candidate ever has a greater power than the seed, its not minimal
			)
			else if (Seeds#i > candidate#k) then (
				seedMinimal = false; -- If our seed ever has a greater power than the candidate, our candidate is safe (for now)
			);
		);
		if seedMinimal then (		-- If our seed is exclusively less than our candidate, we:
			candidate = candidate - Seeds#i; -- Reduce our candidate by our seed, as that will still be invariant
			if (all (candidate, i -> i == 0)) then return {-2, 0}; -- If the candidate fully reduces, we return -2.
			i = i - 1;				-- Reduce our index by one so we can test candidate again. 
		)
		else if candMinimal then (	-- If our candidate is minimal, we may need to discard our seed instead.
			removalSeed := Seeds#i;
			while (all(removalSeed - candidate, i -> i >= 0)) do (
...
genseeds := D -> (
	...
	for i to (n - m - 1) do (                 -- Iterate from 0 to n - m (we don't want our matrix out of bounds)
		candidateSM := submatrix(W, toList(i .. i+m));
		if (determinant candidateSM != 0) then (
			nonZeroSM = candidateSM;    -- If candidateSM has nonzero, it is now our nonZero det submatrix
...
for v in colList do (                   	-- Iterates through all columns we didn't use for nonZeroSM
	...
	seedMatrix      := nonZeroSM | W_v;		-- Matrix we extract the seed invariant from (where W_v is our additional vector)
	signFlip        := 1;
	for i to n do (                 -- This loops lets us remove one of the columns from the matrix to calculate the plücker
			...
			for p from 1 to (Z - 1) do (
				...
				if (not all(candidateSeed, i -> i == 0) and not any(trashList, t -> (candidateSeed == t))) then (
					...
					while (result == -3 or result == -4) do (		-- So we must loop to sort out the seeds and get our candidate & seeds minimized
						editIndex := minimality#2;					-- minimality#2 holds the index of the seed we need to adjust.
						if (result == -3) then (					-- {-3} -> Our seed is not minimal, so we must remove it.
							...
							if (count(newSeed, e -> e != 0) == 1) then ( -- Check if our seed is a pure power of some kind. 
						...
						minimality = seedMinimal(seedList, candidateSeed, editIndex);
						result = minimality#0;
					);
					
					if (result == -1) then (	-- If our candidate is minimal, we add it to the seed list. 
						newCandSeed := drop(minimality, 2);
						if (count(newCandSeed, e -> e != 0) == 1) then ( -- Check if our seed is a pure power of some kind. 
							powerIndex = position(newCandSeed, e -> e != 0);
							purePowers = replace(powerIndex, newCandSeed#powerIndex, purePowers);
						);
						seedList = seedList | newCandSeed;	-- Add our seed to the list. 
					);
					-- If our result is -2, we do nothing because our candidate is bunk.
				);
				trashList = trashList | {candidateSeed}; -- Always add our candidate to the trashList for efficiency.
			);
		);
	);
