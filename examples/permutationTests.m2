...
A = permutationAction(3,{[2,3,1]})
assert(numgens ring A == 3)
assert(coefficientRing ring A === QQ)
assert(#(group A) == 3)

-- shorter permutations are padded with fixed points
C = permutationAction(3,{{[2,1]},[2,3,1]})
assert(#(group C) == 6) -- generates all of S3

print "-- error tests --";
-- rejects non-permutations
try (permutationAction(3,{[1,1,2]}); print "FAIL: should have rejected {1,1,2}") else print "OK: non-permutation correctly rejected"
...
checkAgainstKing = Act -> (
    IG := invariants Act;
    IK := invariants (Act,Strategy=>"King");
...
G5 = permutationAction(5, {[2,1,3,4,5],[2,3,4,5,1]})
print "Goebel:";
elapsedTime IG5 = invariants G5;
print "King (same action):";
elapsedTime IK5 = invariants(G5, Strategy=>"King");
