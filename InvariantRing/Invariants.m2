...
--Gobels thoerem says the ring of invariants for a group that is a subgroup of S_n generated by x_1 * x_2 * ... * x_n 
--and the orbit sums of special monomials, or monomials whose exponents are sorted and start with zero and only differ by 0 or 1.

--Goebel's algorithem 
invariants PermutationAction := List => o -> A -> (
	R := ring A;
	
	-- one orbit sum per special monomial
	candidates := {};
	seen := new MutableHashTable; --keep hashtable of seen list
	scan(specialExponents dim A, v -> 
	if sum v > 0 then scan(unique permutations v, i -> (
        ...
        -- add in the monomial x_1 * x_2 * ... * x_n or produt of variables in the ring
	candidates = append(candidates, product gens R);

        -- Goebels algorithem gives generating set but not minimal so need to get minimal

        -- We walk from low degree to high and use groebner basis to check if a candidate is generated by lower degree monomials

	byDegree := partition(i -> flatten degree i, candidates); -- creates a hash table with degree as key

	S:= {}; -- list of minimal generators
	local Gb; -- grobner basis

	for d in sort keys byDegree do ( -- sort by degree and for each degree check minimal starting from lowest degree
		Gb = gb(promote(ideal S, R), DegreeLimit => d); -- promote puts ideal in the ring R and put in groebner basis object we know we have a degree bound d so we can just do that and its faster
		scan(byDegree#d, f -> (
			g:= f % Gb; -- check for all monomials with degree d if they are minimal
			if not zero g then (
				S = append(S, f); -- if not minimal add it
				Gb = forceGB ( (gens Gb) | matrix{{g}} ) -- update the groebner basis without needed to recompute
			)
		));
	);
S -- return minimal invariants
)
