...
-- Goebel's theorem: the ring of invariants of a subgroup of S_n is
-- generated by x_1 * x_2 * ... * x_n together with the orbit sums of
-- special monomials.
...
invariants PermutationAction := List => o -> A -> (
	...
	for d in sort keys byDegree do ( -- sort by degree and for each degree check minimal starting from lowest degree
		...
		scan(byDegree#d, f -> (
			g:= f % Gb; --reduce f against the invariants kept so far
			if not zero g then (
				S = append(S, f); -- f is not generated by lower-degree invariants, we keep it
				Gb = forceGB ( (gens Gb) | matrix{{g}} )  -- extend the Groebner basis without recomputing it from scratch
			)
		));
	);
...
-- degree d invariants of a permutation action

-- one orbit sum per orbit of degree d monomials. Distinct orbits never
-- share a monomial, so these orbit sums are automatically linearly
...
invariants(PermutationAction, ZZ) := List => o -> (A, d) -> (
    ...
    scan(mons, m -> (
        e := first exponents m;
        if not seen#?e then (
            orb := orbitExponents(A, e);
            scan(orb, i -> seen#i = true);
            candidates = append(candidates, sum(orb, g -> R_g))
        )
    ));
        candidates
