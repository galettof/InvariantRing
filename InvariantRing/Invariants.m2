...
invariants PermutationAction := List => o -> A -> (
    ...
    if o.Strategy == "King" or o.Strategy == "LinearAlgebra"
    then (o >> (lookup(invariants, FiniteGroupAction))) A
    -- otherwise use orbit sums, aka, Göbel's method
        ...
        scan(specialExponents numgens R, v -> 
            if sum v > 0 then scan(unique permutations v, i -> (
                    -- if not seen then calculate orbit and add
                    -- vec to seen and monomial to candidates
                    if not seen#?i then ( 
        ...
        -- We walk from low degree to high and use groebner basis
        -- to check if a candidate is generated by lower degree monomials
        -- creates a hash table with degree as key
        byDegree := partition(i -> first degree i, candidates);
        S:= {}; -- list of minimal generators
        local Gb; -- Groebner basis
        -- sort by degree, for each degree check minimal from lowest degree
        for d in sort keys byDegree do (
            Gb = gb(promote(ideal S, R), DegreeLimit => d);
            scan(byDegree#d, f -> (
                    --reduce f against the invariants kept so far
                    g:= f % Gb;
                    if not zero g then (
                        -- f is not generated by lower-degree invariants, keep it
                        S = append(S, f);
                        -- extend the GB without recomputing from scratch
                        Gb = forceGB ( (gens Gb) | matrix{{g}} )  
                        )
                    ));
            );
